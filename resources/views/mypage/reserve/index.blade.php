	...
	<main class="container" id="new_rsv">
		<section class="main">
			<div class="main-content">
				...
				<div class="phase-step"><img src="{{ asset('img/phase-step1.png') }}" alt="空き状況検索" width="500px"></div>
				<form name="reservationActionForm" id="search-form" method="post" action="{{ route('mypageNewReserveSearch') }}">
					@csrf
					<table class="search">
						<tbody>
							@for ($i = 1; $i <= 3; $i++)
							...
							<!-- 店舗選択エリア(店舗名) -->
							<tr id="shop_area" style="display:none;">
								...
								<td class="td-fourth">
									@for ($i = 1; $i <= 4; $i++)
										<div class="select selecteVertical" style="display:none;" id="shop_area_select{{ $i }}">
											<select class="shop_list_select" name="search_shop[{{ $i }}]" placeholder="店舗名を選択してください" {{ !empty($ary_return_data['search_shop']) && $ary_return_data['search_shop'] == $i ? 'selected' : '' }}>
											</select>
											<span class="arrow"></span>
										</div>
									@endfor
